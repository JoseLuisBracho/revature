	...
	public static void main(String[] args) {
		...
		do {
			...
			case 4:
				...
				break;
			case 5:
				System.out.println("Enter Player Team Name to get the players playing in that Team");
				try {
					String teamname = sc.nextLine();
					List<Player> playersTeamnameList = playerSearchService.getPlayersByTeamName(teamname);
					if(playersTeamnameList!=null && playersTeamnameList.size()>0) {
						System.out.println("There are " + playersTeamnameList.size() + " players in team " + teamname + " printing their details");
						for (Player player : playersTeamnameList) {
							System.out.println(player);
						}
					}
				} catch (BusinessException e) {
			...
			case 7:
				...
				try {
					...
					List<Player> playersDobList;
					try {
						playersDobList = playerSearchService.getPlayersByDob(dob);
