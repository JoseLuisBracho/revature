...
public class PlayerSearchDAOImpl implements PlayerSearchDAO {
	...
	public List<Player> getPlayersByPosition(String position) throws BusinessException {
		...
		try {
			...
			if(playersByPosition.size()==0) {
				throw new BusinessException("No player found playing position: " + position);
			}
		} catch (ClassNotFoundException | SQLException e) {
			throw new BusinessException("Internal Error Ocurred");
		}
	...
	public List<Player> getPlayersByTeamName(String teamname) throws BusinessException {
		// TODO Auto-generated method stub
		List<Player> playersByTeamname = new ArrayList<>();
		try {
			Connection connection = PostgresqlConnection.getConnection();
			String sql = "select p.playerid, p.playername, p.teamid, t.teamname, t.home, p.age, p.position, p.dob, p.contact from \"Test\".player p" +
			             " join \"Test\".team t on t.teamid = p.teamid where t.teamname = ?";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			...
			while (resultSet.next()) {
				...
				playersByTeamname.add(player);
			}
			if(playersByTeamname.size()==0) {
				throw new BusinessException("No player found within the Team: " + teamname);
			}
		} catch (ClassNotFoundException | SQLException e) {
			throw new BusinessException("Internal Error Ocurred");
		}
